...
const send = async (
	...
	const dayData =
		part === 1
			? yearConfig.days[dayNum - 1].part1
			: yearConfig.days[dayNum - 1].part2;
	...
	if (status === Status["SOLVED"]) {
		yearConfig.days[dayNum - 1][part === 1 ? "part1" : "part2"].solved =
			true;
		saveConfig(config);
	...
	if (status === Status["WRONG"]) {
		yearConfig.days[dayNum - 1][
			part === 1 ? "part1" : "part2"
		].attempts.push(dayData.result);
		saveConfig(config);
	}
...
const dev = async (yearRaw: string | undefined, dayRaw: string | undefined) => {
	...
	if (configYear == undefined) {
		...
		config.years.push(
			(configYear = { year: yearNum, days: aocAutomationDaysJSON() }),
		);
	...
	chokidar
		.watch([path.join("src", "utils"), path.join("src", year)], {
			ignoreInitial: true,
		})
		.on("add", reload)
		.on("change", reload);
